 ...
 *   - Static revision：CACHE_NAME = swiss-trip-v21-7d-final-2027；不接受舊 cache 為 current；ASSETS 完整
...
const NEW_CACHE = "swiss-trip-v21-7d-final-2027";
...
(async () => {
  ...
  check("sw.js 標示 Final Deployment Revision 註解", /Final Deployment Revision/.test(swSrc));
  ...
  section("3. Install behavior");
  {
    ...
    await checkA("install 開啟新 cache（swiss-trip-v21-7d-final-2027）", async () => { await ev._waited; return sw.log.opened.indexOf(NEW_CACHE) !== -1; });
  ...
  {
    const sw = loadSW({ existingKeys: [OLD_CACHE, "swiss-trip-v21-7c-2027", NEW_CACHE] });
    ...
    check("恰好刪除 2 個舊 cache", sw.log.deleted.length === 2);
  ...
  // 5b. 同源 cache miss → fetch → 成功 put 進新 cache
  {
    ...
    await checkA("成功 fetch（200 basic）→ put 進新 cache（swiss-trip-v21-7d-final-2027）", async () => { await flush(); return sw.log.put.length === 1 && sw.log.put[0].name === NEW_CACHE; });
  }
  // 5c. 跨域 → 不攔截
  {
    const sw = loadSW({});
